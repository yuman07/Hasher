...
- **国际化** — 一个扁平的 `messages` 对象包含 `en`/`zh` 键值；启动时通过 `navigator.language` 决定界面语言（不提供手动切换）。
...
- **状态** — 单个 `state` 对象持有文件映射、设置、语言和主题。仅算法选择和哈希大小写通过 `localStorage` 持久化；语言和主题每次启动都重新从系统读取。
...
- **偏好持久化** — 算法选择和哈希大小写存储在 `localStorage`，每次启动自动恢复；主题和语言每次启动都重新从系统读取，与 Rust 后端解耦
